...
const isCodeUser = async (req, res, next) => { // 1. Get code params
    ...
    try { // Init transaction mysql
        ...
        if (codeDB === undefined || codeDB.code !== code) {
            ...
            // Send status
            return res.status(406).json({info: msgInfo});
        }
